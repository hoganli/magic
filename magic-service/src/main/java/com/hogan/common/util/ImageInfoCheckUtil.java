...
/**
 * @ClassName:ImageInfoCheckUtil
 * @Description:抽取自ImageInfo.java
 * 用于判断现在主流的图片内容（check()方法）
 * "JPEG", "GIF", "PNG", "BMP", "PCX", "IFF", "RAS", "PBM", "PGM", "PPM", "PSD"
 * @Author:shou-quan.tang
 * @Date:2018/1/22 16:23 星期一
 */
public class ImageInfoCheckUtil {
    /**
...
/*	public static final int COLOR_TYPE_UNKNOWN = -1;
    ...
    public static void main(String[] args) {
        ...
        // 使用此工具类读取图片输入流，调用check()方法字节判断文件内容是否为图片
//        ImageInfoCheckUtil ii = new ImageInfoCheckUtil();
//        FileItem item = null;
//        try {
//            ii.setInput(item.getInputStream()); // in can be InputStream or RandomAccessFile
//            ii.setDetermineImageNumber(true); // default is false
//            ii.setCollectComments(true); // default is false
//            if (!ii.check()) {
//                System.out.println("此文件不是图片文件");
//            }else{
//                System.out.println("此文件是图片文件");
//            }
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
    ...
    private static void print(String sourceName, ImageInfoCheckUtil ii, boolean verbose) {
        if (verbose) {
    ...
    private static void printCompact(String sourceName, ImageInfoCheckUtil imageInfo) {
        final String SEP = "\t";
        System.out.println(
                sourceName + SEP +
    ...
    private static void printVerbose(String sourceName, ImageInfoCheckUtil ii) {
    ...
    private static void run(String sourceName, InputStream in, ImageInfoCheckUtil imageInfo, boolean verbose) {
